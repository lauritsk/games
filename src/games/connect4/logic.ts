import { required, type Difficulty, type RandomSource } from "@shared/core";
...
export function chooseConnect4BotColumn(
  ...
  if (difficulty === "Hard")
    return safeShapeMove(board, valid) ?? bestShapeMove(board, valid) ?? randomMove(valid, rng);
...
export function findConnect4TacticalMove(
  ...
  valid = playableConnect4Columns(board),
): number | null {
  assertConnect4Board(board);
...
export function dropConnect4DiscInPlace(
  board: Connect4Cell[][],
  column: number,
  player: Connect4Player,
): number | null {
...
export function findConnect4Win(
  board: Connect4Cell[][],
  row: number,
  column: number,
  player: Connect4Player,
...
function bestShapeMove(board: Connect4Cell[][], valid: number[]): number | null {
  ...
  return (
    [...valid].sort(
      (a, b) =>
        scoreMove(board, b, connect4Bot) - scoreMove(board, a, connect4Bot) ||
...
export function assertConnect4Board(board: Connect4Cell[][]): void {
  if (board.length !== connect4Rows || board.some((row) => row.length !== connect4Columns))
    throw new Error("Invalid Connect 4 board shape");
}
